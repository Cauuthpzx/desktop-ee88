#include "core/register_widget.h"
...
void RegisterWidget::setup_ui()
{
    ...
    setStyleSheet(QString("RegisterWidget { background: %1; }").arg(bg_deep));
    ...
    auth_container->setFixedSize(900, 580);
    auth_container->setStyleSheet("background: transparent; border: none;");
    ...
    // Title
    auto* title = new QLabel(QString::fromUtf8("Tạo tài khoản"));
    ...
    auto* subtitle = new QLabel(QString::fromUtf8(
        "Đăng ký để bắt đầu sử dụng MaxHub"
    ));
    ...
    pwd_lbl->setStyleSheet(label_style);
    form_layout->addWidget(pwd_lbl);
    form_layout->addSpacing(4);

    m_password_input = new QLineEdit;
    m_password_input->setPlaceholderText(QString::fromUtf8("Tối thiểu 8 ký tự"));
    ...
    // ── Confirm Password ──
    auto* cfm_lbl = new QLabel(QString::fromUtf8("Xác nhận mật khẩu"));
    cfm_lbl->setStyleSheet(label_style);
    form_layout->addWidget(cfm_lbl);
    form_layout->addSpacing(4);

    m_confirm_input = new QLineEdit;
    m_confirm_input->setPlaceholderText(QString::fromUtf8("Nhập lại mật khẩu"));
    m_confirm_input->setEchoMode(QLineEdit::Password);
    m_confirm_input->setStyleSheet(input_style);
    m_confirm_input->setFixedHeight(42);

    auto* key_icon = new QLabel(m_confirm_input);
    key_icon->setText(QString::fromUtf8("🔒"));
    key_icon->setFixedSize(20, 20);
    key_icon->move(12, 11);
    key_icon->setStyleSheet(icon_style);

    connect(m_confirm_input, &QLineEdit::returnPressed, this, &RegisterWidget::on_submit);
    form_layout->addWidget(m_confirm_input);
    form_layout->addSpacing(12);

    // Error box (dark theme)
    m_error_box = new QWidget;
    m_error_box->setStyleSheet(
    ...
    m_submit_btn = new QPushButton(QString::fromUtf8("Đăng ký"));
    ...
    connect(m_submit_btn, &QPushButton::clicked, this, &RegisterWidget::on_submit);
    ...
    auto* footer_text = new QLabel(QString::fromUtf8("Đã có tài khoản?"));
    footer_text->setStyleSheet(QString(
        "color: %1; font-size: 12px; background: transparent; border: none;"
    ).arg(text_muted));
    footer_layout->addWidget(footer_text);

    auto* login_link = new QPushButton(QString::fromUtf8("Đăng nhập"));
    login_link->setFlat(true);
    login_link->setCursor(Qt::PointingHandCursor);
    login_link->setStyleSheet(QString(
        "QPushButton { color: %1; font-size: 12px; font-weight: 600; "
        "border: none; background: transparent; padding: 0; }"
        "QPushButton:hover { color: %2; }"
    ).arg(cyan, "#00E5FF"));
    connect(login_link, &QPushButton::clicked, this, &RegisterWidget::switch_to_login);
...
void RegisterWidget::on_submit()
{
    ...
    const auto confirm = m_confirm_input->text();
    ...
    if (password.length() < 8) {
        set_error(QString::fromUtf8("Mật khẩu phải có tối thiểu 8 ký tự"));
        return;
    }

    if (password != confirm) {
        set_error(QString::fromUtf8("Mật khẩu xác nhận không khớp"));
        return;
    }
    ...
    m_api->register_user(username, password, [this](bool success, const QJsonObject& data) {
        ...
        if (success && data.contains("token")) {
            ...
            const auto error = data.value("message").toString(
                QString::fromUtf8("Đăng ký thất bại")
            );
            set_error(error);
        }
    });
...
void RegisterWidget::set_error(const QString& msg)
{
    m_error_label->setText(msg);
    m_error_box->setVisible(true);
}

void RegisterWidget::clear_error()
{
    m_error_box->setVisible(false);
    m_error_label->clear();
}
